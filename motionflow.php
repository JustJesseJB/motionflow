...
function motionflow_include_files() {
    ...
    if (!file_exists(MOTIONFLOW_PLUGIN_DIR . 'includes/api')) {
        mkdir(MOTIONFLOW_PLUGIN_DIR . 'includes/api', 0755, true);
    }
    
    // Create the Frontend Main class file if it doesn't exist
...
class Main {
    ...
    // Create the API Main class file if it doesn't exist
    $api_main_file = MOTIONFLOW_PLUGIN_DIR . 'includes/api/class-main.php';
    if (!file_exists($api_main_file)) {
        $api_main_content = '<?php
/**
 * API Main Class
 *
 * @package    MotionFlow
...
class Main {
    ...
    public function add_to_cart($request) {
        ...
        file_put_contents($api_main_file, $api_main_content);
    }
    
    // Include the new frontend and api classes we just created
    require_once MOTIONFLOW_PLUGIN_DIR . 'includes/frontend/class-main.php';
    require_once MOTIONFLOW_PLUGIN_DIR . 'includes/api/class-main.php';
